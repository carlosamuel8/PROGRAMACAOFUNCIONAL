...
fitValues xs lim index =[x | x <- [0..lim], fit(xs, lim) index x]

solve :: (String, Int) -> [Int] -> Int -> Maybe String
solve (xs, lim) holes hindex
    | sizeH==hindex = Just xs
    | (length) fvalues== 0 = Nothing
    | otherwise = 
        if length(res) == 0 then Nothing 
        else head res
    ...
    where
        ...
        poss = [solve (set xs index (dig2char x), lim)holes (hindex+1) | x <- fvalues]
        res = [x | x <- poss, isJust x]
